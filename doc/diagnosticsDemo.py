...
def trigger_camera(t, exposure, context, origin=None,
    ...
    return tl.update("trigger_camera",[[t,1],[t+exposure,0]], context=context, origin=origin, **kwargs)
...
def flash_light(t, exposure, context, origin=None, **kwargs) :
    ...
    return tl.stack(
        tl.update("AOM_imaging",[[t,1],[t+exposure,0]],context=context,origin=origin,**kwargs),
        tl.update("shutter_imaging",[[t-exposure*(sf-1)-ex.constants.AI.lag_shutter_on,1],[t+exposure*sf,0]],context=context,origin=origin),
...
def expose_camera(t, exposure, context, origin=None, **kwargs) :
    return tl.stack(
        ...
        trigger_camera(t-camera_exposition_margin, exposure+2*camera_exposition_margin, context, origin)
    )
...
def take_image_plus_Bg(t, exposure, delayBg, context, origin, **kwargs) :
    return tl.stack(
        ...
        trigger_camera(t+delayBg, camera_exposition_from_AOM_exposition(exposure), context, origin), # taking Bg_At image
    )

def imaging_absorption(t, exposure, origin, context="AI", delayBg=50e-3, delayLi=0.2, AOM_offAdvance=0.1, exposureBlow=1e-2, **kwargs) :
    context = "imaging_absorption"
    return tl.stack(
        tl.update(AOM_imaging=0,t=t-AOM_offAdvance,context=context,origin=origin, **kwargs), # initializing the AOM
        take_image_plus_Bg(t, exposure, delayBg, context, origin), # taking At + Bg_At image
        flash_light(t+delayBg+delayLi/2.,exposureBlow,context,origin) if exposureBlow is not None else None, # blow out the atoms in between
        take_image_plus_Bg(t+delayLi, exposure, delayBg, context, origin), # taking Li + Bg_Li image
#        tl.anchor(t+delayLi+2*delayBg,context=context,origin=origin)
    )


def prepareSample(initFunction=init, finishFunction=finish, **kwargs) : return ex.prepareSample(initFunction=initFunction, finishFunction=finishFunction, **kwargs)
