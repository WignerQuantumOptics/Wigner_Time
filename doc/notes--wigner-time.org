...
Emphasize that by using cascading functions we can move up and down the layers of abstraction. New functions can be very 'easy' and high-level (MOT=on), but can also drop down to the bottom layer (5V at 6ms...).
