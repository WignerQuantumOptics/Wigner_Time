...
# TODO: Change this file from random ideas to structured notes on code.

* API policies
Hide everything that has no API commitment under `_internal`. 

Major changes in API (post publication) will include a backwards-compatibility namespace? 

** package numbering
standard major.minor.patch package versioning.
- major - API changes (breaking changes, if any)
- minor - new features
- patch - bug fixes (no expected user downsides or surprises)
...
** clear ways of importing timelines from file
