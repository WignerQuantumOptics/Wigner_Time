...
- **`sec:parameter_scan` calls `adwin.create` pure** (main.tex:1446): "it constructs a backend object without side effects". It has them: it writes `Par_1..3` and the data arrays on the machine, and with no `machine` it opens a connection to device 1. The listing works regardless, because each upload finishes before the start that follows it. Found 2026-09-23 and **not changed**. It bears on roadmap step 5 at #94, which proposes that `create` require its machine and return a record of what it uploaded rather than the machine itself. That would also change main.tex:818 ("returns a machine object that can be used to start and stop processes") and the `backend.Start_Process(1)` of `sec:demonstration` (main.tex:1365–1366). The maintainer's decision.
