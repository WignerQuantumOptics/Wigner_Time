...
- **`sec:stacking` calls `trigger_camera(0.0,1e-3)`**, a few lines below a definition of `trigger_camera` whose `context` has no default. As printed it raises `TypeError`. The required `context` is right, and not only for the paper: a placed stage that inherited its context would take `ADwin_Finish` when attached to a finished timeline. So the call should change, not the signature. Found 2026-09-23 and not yet changed;
- **`sec:interweaving` ends "For further examples, see `sec:demonstration`, `sec:parameter_scan`"**, but the `sec:demonstration` listing has no interwoven stage: its only `origin` is `MOT`'s absolute `0.0`. `sec:parameter_scan` does pass `origin` through, but shows neither the imaging function nor a context value. The code demo now has one (`trigger_camera`, placed at `origin="molasses"` into the finished demo timeline by `test_demo.py`; #53), so there is something for the paper to show. Found 2026-09-23 and not yet changed;
