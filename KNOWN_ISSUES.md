...
**Refined 2026-09-23: the exemption is now a difference of defaults, not a special-cased function.** Raised by the maintainer: the fix above withheld a value origin from `df_1` (the explicitly-started rows) via a bare `None` literal hand-written into `ramp`'s body — not read from any table `config` exposes, not reachable through `origin=`, and indistinguishable at the call site from an ordinary default. Whatever `ramp` did with a stated start was invisible unless you read `ramp`'s source, and a caller had no way to ask for anything else. An intermediate attempt made the override conditional on whether the caller's own `origin=` happened to state a value slot — but that reintroduced the same complaint one level down: it was still a hand-written condition (`origin__value_stated = ...`) reimplementing, in `ramp`, a distinction `wt_origin.auto` already draws internally, and it meant the identical `origin=["anchor", "variable"]` behaved differently depending on whether that value came from the caller's own text or from `auto`'s default-filling — an invisible, call-site-textual difference standing in for what should be a difference of specification.

**Fixed by recognising what an explicitly-stated start point actually is: a value that is already absolute, exactly like `update`'s and `anchor`'s.** So it resolves against the same table they do — `config.ORIGIN__DEFAULTS`, whose value slot is `None` — rather than against `ORIGIN__DEFAULTS__RAMP`, whose value slot is `"variable"` precisely so an *inferred* start can be filled in. `ramp` now calls the ordinary `wt_origin.auto` twice, once per row category, differing only in which named table it is given:
...
No new parameter, no new config table, and no bespoke "was this explicit" check anywhere: `auto`'s own contract (a default only fills a slot the caller left `None`, and never overrides one that is not — see its docstring) is what makes a caller's own explicit value origin still take effect on a stated start, for free, without `ramp` having to ask the question itself. The distinction between "df_1 gets no implicit value origin" and "df__no_start_points gets `'variable'`" is now visible by reading `config.py` — it is the ordinary difference between the table `update`/`anchor` use and the table `ramp` uses for an inferred point — not a branch inside `ramp`.

```python
# lockbox_MOT__V last known at 0.2
tl.ramp(base, lockbox_MOT__V=[[0.0, 0.0], [0.5, 5.0]], origin="stage1")
#   start stays 0.0 -- resolves against ORIGIN__DEFAULTS, value slot None

tl.ramp(base, lockbox_MOT__V=[[0.0, 0.0], [0.5, 5.0]], origin=["stage1", "variable"])
#   start comes out as 0.2 -- the caller's own explicit value slot is honoured,
#   because ORIGIN__DEFAULTS never had anything to override
```

Ordinary use (the 2-D form with no `origin=` opinion of its own) is unaffected. One existing test needed adjusting because it had, coincidentally, spelled out the *default* value origin explicitly: `test_ramp_start_stated_explicitly` passed `origin=["anchor", "variable"]`, and `"variable"` is also `ORIGIN__DEFAULTS__RAMP`'s own value slot for `ramp`. That coincidence made no difference under the September fix, since the value slot was ignored outright for `df_1` regardless of what it said — but once the two tables genuinely differ, writing that slot at all resolves against the table that has an opinion about it. The test is changed to `origin="anchor"` (a time reference, value slot left unstated) to keep pinning the default case, and a new test, `test_ramp_start_stated_explicitly_with_explicit_value_origin`, pins the explicit-value-origin case with the original `["anchor", "variable"]` call. Both, plus the rest of the origin/ramp suite (78 cases across `test_timeline_ramp.py`, `test_origin.py`, `test_origin_defaults.py`, `test_origin_slots.py`), pass, as does the full suite (300 passed, unchanged from before this entry apart from the one rewritten test and one added).

**Refined again, 2026-09-23 — the two-table split itself is gone.** Raised by the maintainer: the 2026-09-23 fix above still let one thing decide a row's value-origin default that had nothing to do with `origin=` — *which of the two input shapes the caller used for that variable*. `df_1` (2-D, explicit start) always resolved against `ORIGIN__DEFAULTS` (value slot `None`); `df__no_start_points` (1-D, inferred start) always resolved against `ORIGIN__DEFAULTS__RAMP` (value slot `"variable"`). Two named, editable tables made the *tables* visible, but the *choice* between them was still keyed off shape, invisibly, exactly the pattern this whole issue exists to remove: the same `origin="anchor"` resolved differently depending on a fact `origin=` itself could not see.

**The fix, this time, is to remove the choice rather than name it better.** `ramp` now resolves every start row — 2-D or 1-D — against the single table `ORIGIN__DEFAULTS__RAMP`, through one call. A stated 2-D start is offset by the variable's current value *by default*, exactly like an inferred one; there is no longer a shape-driven exemption of any kind.

That closes the silence, but it reopens A8's original complaint in the other direction: writing `lockbox_MOT__V=[[0.0, 1.0], [0.5, 3.0]]` no longer protects the stated `1.0` from being offset, which is the very thing `tab:rampExamples` promises the 2-D form is *for*. Something has to let a caller ask for a literal start, and it has to be spelled in `origin=`, not inferred from shape. The maintainer's proposal, adopted here: split the parameter's own default away from `None`.

- `update`, `anchor` and `ramp` now default to `origin=wt_config.ORIGIN__INFER` (a new sentinel, the literal string `"INFER"`), not `origin=None`. Nothing observable changes for any existing call that omits `origin=` — grepped across `test/`, `tests/`, `src/` and `doc/`: every occurrence of `origin=None` in the whole tree was a function *signature* default, never a call site, so no caller anywhere was relying on writing it explicitly (there was no way to, since it was indistinguishable from omitting it).
- `origin=None`, now reachable only by writing it, means the opposite of the default: *no* origin resolution at all. `origin__defaults` is never consulted; the pair stays `[None, None]`, which `internal.origin.update` already treated as a complete no-op — the stated coordinates return exactly as given, in whichever slots the caller filled in.
- A new wrapper, `internal.origin.auto_or_off`, is where this is implemented — not inside `auto` itself, which keeps its own long-standing contract unchanged (a bare `None` passed directly to `auto` still means "run the defaults", exactly as `auto`'s own tests, called independently of the public functions, continue to pin). `auto_or_off` translates the public sentinel (`ORIGIN__INFER` → the `None` `auto` has always read as "infer"; an explicit `None` → `[None, None]`, bypassing `auto` altogether) before anything reaches `auto`.
- `anchor`'s own pre-resolution of `origin` (calling `auto` itself before handing a concrete pair to `update`, which then resolved it *again*) is removed. It was harmless while every `None` meant one thing everywhere, since re-running the old `auto` on its own output was idempotent; under the new scheme it was not merely redundant but wrong, because `update`'s `auto_or_off` cannot tell an already-resolved `[None, None]` (meaning "off", if `anchor` had just computed that) from a still-unresolved one (meaning "infer"), and would silently re-infer it. `anchor` now passes its own `origin` parameter straight through, so `update` is the single place any of the three ever resolves it.

**A sharper cost than expected, found via the existing test suite rather than anticipated.** `test_an_unset_variable_can_state_both_ends` (`test_origin_defaults.py`) stated both ends of a brand-new variable with no history, relying on the old `ORIGIN__DEFAULTS`-for-`df_1` behaviour to skip a value lookup that would otherwise fail outright. Under the unified table this raised: `ValueError: No previous value of 'fresh__A' to start from`. A blanket `origin=None` is *too* blunt a fix for it, and revealed a distinction worth recording: `None` turns off *both* slots, but this call still wanted its start time anchored normally (`tline`'s anchor sits at 3.5, not 0.0) — only the value needed to be literal. The already-existing per-slot vocabulary (`None` in one slot = defer to that slot's default; `0.0` = absolute, no shift — both predate this fix, see `internal.origin.auto`'s docstring) says exactly that: `origin=[None, 0.0]`. The two mechanisms compose as designed: the blanket sentinel for "protect everything", the per-slot pair for "protect just this".

`ramp`'s own two `internal.origin.update` calls (one for `df_1`, one for `df__no_start_points`) are kept separate even though both now share one resolved `origin` — each still computes its own `time__max__relative` bound from only its own rows (B2/#109), and concatenating the frames before resolving would have widened that bound to the union's earliest row, tightening what either half of a mixed call could see as its own "previous" value. That split is a correctness detail of *when* the origin is applied, not *which* origin is resolved, and nothing about today's change touches it.

Verified: the full suite (301 passed — two ramp tests renamed/rewritten, one origin-defaults test's call updated, one new test added, pinning `origin=None`'s literal-start behaviour) shows the same 9 pre-existing, unrelated failures as the pre-change baseline (`ADwin` import, parquet/feather round-trips, one `test_convert` case) and none new. The shipped demo (`demo.full_experiment.timeline__demo`, 99 rows) is byte-identical in `time`/`variable`/`value`/`context` before and after — it uses only the 1-D shorthand throughout, so today's change touches nothing it does.

**A sitewide opt-in back to the old convention, added the same day.** The maintainer wanted the shape split available as a convenience for a colleague, without reopening the door that was just closed: nothing may branch on shape *silently*. Two spellings were considered and rejected before this one. Gating on `origin == wt_config.ORIGIN__INFER` inside `ramp` (i.e. "do the shape thing whenever the caller left `origin` at its default") was the first: it makes the identical stated start protected when nothing is written and offset the moment the caller also names an unrelated time reference like `origin="stage1"` — worse than the original shape-coupling, because now *whether `origin=` was touched at all*, for any reason, silently decides value protection. A new value in `origin=`'s own vocabulary (e.g. `origin="INFER_BY_SHAPE"`) was the second: rejected only because the maintainer didn't want another `origin` option, not because it was unsound — it would have worked, opt-in and visible at the call site.

**Settled on instead: `wt_config.ORIGIN__INFER_BY_SHAPE`, a plain boolean, `False` by default.** In `ramp`, `if origin == wt_config.ORIGIN__INFER and wt_config.ORIGIN__INFER_BY_SHAPE:` reinstates the old two-table split (`df_1` against `ORIGIN__DEFAULTS`, `df__no_start_points` against `ORIGIN__DEFAULTS__RAMP`) for that one condition only; otherwise (including any `origin=` the caller writes, always, regardless of the switch) both resolve uniformly against `ORIGIN__DEFAULTS__RAMP`, exactly as above. This is honest about what kind of thing it is: not a per-call option, but a site's standing convention, read fresh at every call — flipping it changes every bare-`origin` `ramp` call in the running process, not one. A caller who wants a stated value protected on one particular call already has `origin=None` (or a per-slot `[None, 0.0]`) for that, unaffected by this switch either way.

Two new tests exercise it (`test_ramp_start_stated_explicitly_is_protected_with_infer_by_shape`, `test_infer_by_shape_does_not_affect_an_explicit_origin`), via a fixture that flips the flag on and restores it afterwards regardless of outcome, since it is process-wide state that must not leak between tests. Full suite: 303 passed, same 9 pre-existing failures, demo still byte-identical (the flag defaults `False`).

**Flipped the same day, on request: default `True`, condition widened to `origin is not None`.** Two changes, made together because the first only works with the second. `ORIGIN__INFER_BY_SHAPE` now defaults `True` rather than `False` — "for humility's sake": `ramp`'s long-standing convention stays what a caller gets unless a site deliberately opts out, rather than making this session's refinement the default for everyone from the day it lands. And the gate in `ramp` changed from `origin == wt_config.ORIGIN__INFER` to `origin is not None`, so the shape split now applies to *any* `origin` the caller writes — a bare call, `origin="anchor"`, `origin="stage1"`, anything — except an explicit `None`.

Widening the gate this way is what makes flipping the default sound rather than reopening the earlier objection. The version gated on `origin == ORIGIN__INFER` alone (rejected two entries above) made the identical stated start protected when nothing was written and offset the moment the caller named an unrelated time reference — value protection silently keyed off *whether `origin=` was touched at all*. Gating on `origin is not None` instead removes that seam entirely: every `origin` except the one spelling that already means "no resolution, take everything literally" gets the same treatment, so nothing about writing a time-only origin changes anything. This is, in fact, exactly `ramp`'s original (2026-09-18) behaviour — a stated 2-D value protected unless the caller's own `origin` names a value origin explicitly — with one new thing added on top: `origin=None` as an explicit, always-honoured "no resolution at all" escape hatch, which did not exist before this session at all.

Net effect: `wt_config.ORIGIN__INFER_BY_SHAPE = True` (default) restores `ramp`'s historical behaviour for every call that doesn't ask for something else via `origin=None`; `= False` asks for this session's refinement (fully uniform, shape-blind) as a site's standing choice instead. Tests reworked accordingly: `test_ramp_start_stated_explicitly_is_protected_by_default` pins the restored default with an explicit time-only `origin="anchor"` (not just a bare call, to show the widened gate), `test_ramp_start_stated_explicitly_with_explicit_value_origin` and `test_ramp_start_stated_explicitly_can_be_kept_literal` are unchanged in effect, and a new `infer_by_shape_off` fixture (flips the switch `False`, restores it after) backs three tests mirroring the same three cases with the switch off. Full suite: 304 passed, same 9 pre-existing failures, demo still byte-identical.

**Not yet done:** `docs/paper/main.tex` documents the old `origin=None`-is-the-default reading in several places (§sec:origin's "The default" paragraph, the `update`/`anchor`/`ramp` signature listings, §sec:demonstration, §sec:design_rationale, and `tab:rampExamples`'s "taken as written" row) and has not been touched by this fix — it is the published manuscript, not dev-tracking, and updating its prose is left until confirmed with a colleague rather than done here.

**Extended to context inheritance, 2026-09-24 — same sentinel/off split, for a different silent behaviour.** The maintainer asked for the origin work's exact shape applied to context: "only do the inheritance when context says `'INFER'` and make that the default instead of `None`", then sharpened it — an explicit `context=None` should be a genuine "off" state, "just use the default context, which I think is the empty string", not merely a renamed spelling of today's default.

The overloading was the same shape as A8's: `context=None` was already the parameter default for `create`, `update`, `anchor` and `ramp`, already meaning "inherit an unstated row's context from wherever the timeline it joins last left off" (`internal.timeline.inherit.context`'s `context is None` branch). There was no way to *ask* for the opposite — an unstated row that stays in the plain default context, the empty string, regardless of what the timeline it joins was doing.

- `create`, `update`, `anchor` and `ramp` now default to `context=wt_config.CONTEXT__INFER` (a new sentinel, the literal string `"INFER"`), not `context=None`. Nothing observable changes for any existing call that omits `context=`: grepped across `test/` and `src/` for `context=None` written at a call site (as opposed to a signature default) and found none passing into these four functions — the one real hit, `demo.full_experiment.pull_coils`'s own `context=None` default forwarded to `tl.ramp(context=context, ...)`, is a site convenience wrapper, not a call site, and is addressed below.
- `context=None`, now reachable only by writing it, means the opposite of the default: *no* inheritance. The new rows are left with the empty-string context row construction already gives an unstated row (`internal.timeline.input.__ensure_time_context`'s existing placeholder), and nothing is copied from the timeline being joined.
- A new function, `internal.timeline.inherit.resolve`, is where this is implemented — not inside `inherit.context` itself, which keeps its own long-standing contract unchanged (a bare `None` passed directly to it still means "inherit", exactly as its own direct callers — including `test_createInheritContext`, which calls `tl._populate_timeline` directly — continue to rely on). `resolve` translates the public sentinel (`CONTEXT__INFER` → `None`, which `inherit.context` has always read as "inherit"; an explicit `None` → `""`, the literal empty string) before anything reaches `inherit.context` or row construction. Translating to `""` rather than passing `None` through is what makes the "off" state work with zero changes to `inherit.context`'s own logic: `""` is not `None`, so its "infer from previous" branch does not fire, and it falls through to the existing no-op branch — the rows already carry `""` from construction.
- Each public function resolves `context` exactly once, at its own single point, mirroring `auto_or_off`'s placement for `origin`: `create` and `update` each call `resolve` themselves, immediately before calling the shared `_populate_timeline` (whose own `context=None` signature default is untouched, and still means "inherit" for any direct caller, same as `auto`'s own contract was left alone for origin). `ramp` calls `resolve` itself too, since it builds its rows directly and does not route through `_populate_timeline`. `anchor` does not resolve `context` at all — it forwards its own parameter straight through to `update`, exactly as it already does for `origin`, for the identical reason: a second resolution could misread an already-resolved value. (Checked precisely here: it would not actually misfire, since `""` is neither the sentinel nor `None` and passes through `resolve` unchanged — but the "resolve once, at the entry point" discipline is kept anyway, for the same reasons it was kept for origin.)

**One real site-code fix, found only by checking the demo stayed byte-identical.** `demo.full_experiment.pull_coils` has its own `context=None` default, forwarded as `context=context` into `tl.ramp`, and `magnetic_trapping` calls it once relying on that default (`pull_coils(duration__strengthen, ls, us, t=duration__initial)`, no `context=` given). Under the old scheme this meant "inherit"; under the new one, forwarding a bare `None` explicitly now means "off" — so the demo would have started using the empty context for that stage's rows instead of inheriting `"magnetic_trapping"`. Fixed by giving `pull_coils` the same sentinel default, `context=wt_config.CONTEXT__INFER`, so a caller leaving it unstated gets exactly what it always got. This is the context-work equivalent of the `anchor`/`update` double-resolution fix in the origin work: a convenience wrapper's own default has to track the library's sentinel, once the parameter it forwards stops being a plain, undifferentiated `None`.
