...
\end{minted}
%
This function accepts a wide range of input formats through the \python{**vtvc_dict} keyword argument, whose possibilities are laid out in \cref{tab:inputSpecs}. The arguments \python{t} and \python{context} serve as defaults for cases where time and context are not specified individually for each variable.

A variable is therefore always named as a keyword. Since that keyword namespace \emph{is} the variable namespace (\cref{sec:forwarding}), there is deliberately no second, positional way of naming one: an unrecognised keyword is read as a variable rather than rejected, so a parallel syntax would be a second thing for it to be confused with. Where the names are computed rather than written, the usual \python{**} unpacking applies, as in \python{create(**{name: value for ...})}.
...
\begin{table}
    ...
    \end{tabular}
    \caption{The input formats for \python{create}. Rows may also be assembled positionally rather than named, which the package uses internally when expanding a ramp into its points; see the API documentation \cite{wignertime_docs}.}
...
\paragraph{What a ramp refuses} A ramp is a transition between two distinct instants, and the two ways of not supplying one are refused rather than interpreted. A \python{duration} of zero leaves both boundaries on the same instant; a negative one places the end before the start, where -- since \python{expand} orders each ramp's boundaries in time -- it would quietly exchange them, so that the variable finished at its previous value rather than at the target and the transition was laid over whatever preceded it. Both raise, naming the variables concerned. A ramp whose \emph{value} does not change is a different matter and is not refused: it is a hold, it occupies time, and discarding it would shorten the timeline and draw everything after it forward. The redundant rows it expands into are removed again by the filtering of \cref{sec:adwin} before they reach the hardware, so the cost is borne by the device layer alone. Finally, because a ramp begins wherever its variable currently sits, a variable with no previous entry has no starting point at all; this too raises, and the remedy is either to set the variable beforehand or to state both ends explicitly, as in the last row of \cref{tab:rampExamples}. Where they are stated, they are taken as written: the value origin is resolved only for those variables whose starting point had to be inferred.
...
\end{minted}
%
The two coincide until some stage writes past its own anchor, and then they do not: in the example of \cref{sec:demonstration} they differ by \(\sim0.1\,\)s from \python{optical_pumping} onwards. Which is meant is therefore stated at the call site rather than defaulted to.
...
\end{minted}
Since no timeline is supplied, this returns a deferred function that can be slotted into any later timeline. Note the explicit \python{timeline=None}: a stage declares the placement arguments it forwards, exactly as the core functions do, and it is this that makes it both directly callable and composable. A stage should not collect them in \python{**kwargs} -- there, the keyword namespace of the terminal \python{create} or \python{update} call \emph{is} the variable namespace (\cref{sec:functions}), so a mistyped argument is silently read as a new variable rather than rejected. The one place an open namespace is wanted is \python{default_state}, where that reading is the entire point (\cref{sec:forwarding}).
...
\end{minted}
%
Routing is strict. A keyword that names no stage, or that names one but is not a parameter of it, raises rather than being quietly dropped -- a dropped keyword would leave the stage on its default, which is a physically different sequence that nonetheless runs. The check is derived rather than configured: a split is accepted when the target declares the parameter, or when it collects \python{**kwargs} and is therefore deliberately open, as \python{default_state} and the functions wrapping it are (\cref{sec:forwarding}). Matching is anchored to the start of the keyword and to an underscore boundary, so a parameter that merely contains a stage name is not captured by it, and where two stage names both anchor the longer one is preferred.
%
...
\python{"last"} and \python{"anchor"} are defined temporally and are admissible in the time slot only, as is a context name; the reason is given in the caption of \cref{fig:origin}. Because these three strings are resolved before any variable or context of the same name, such a name would be unreachable as an origin, and the package therefore refuses it where it is written.
...
Keyword forwarding provides a third way. \python{default_state} accepts \python{**kwargs} and passes them to its terminal \python{create} or \python{update} call, where -- because that call's keyword namespace \emph{is} the variable namespace (\cref{sec:functions}) -- an unrecognised keyword is read as a variable-value pair. The two stages that wrap it, \python{init} and \python{finish}, forward \python{**kwargs} in the same way, so a keyword that neither consumes falls through to the default state and initialises a variable at both ends of the run.

The open namespace stops there, and deliberately. An ordinary stage declares the placement arguments it forwards and nothing else (\cref{sec:stacking}), because in a stage the same mechanism is a liability rather than a feature: a mistyped argument would be read as a new variable and silently written into the middle of the experiment, where -- unlike in the default state -- there is no reason to want one. Confining \python{**kwargs} to this one path is also what allows \python{cascade} to check its routing against the target's signature.
...
Nothing here is a library feature. It is ordinary Python keyword forwarding, available because stages are ordinary functions rather than objects with fixed interfaces, and because the library keeps one namespace deliberately open: the terminal \python{**vtvc_dict}. The cost of that openness is that a misspelled keyword arriving at the terminal call is still a legal variable name, and so creates a variable rather than raising. The connection table is what bounds the consequence -- a variable with no matching connection cannot reach the hardware, and is removed during conversion (\cref{sec:connectionlayer}) -- which is a further reason to treat that table, rather than the stage functions, as the authoritative statement of what the apparatus is.
