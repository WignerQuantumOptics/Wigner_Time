...
%\lhead{\colorbox{scipostblue}{\bf \color{white} ~Wigner Time }}
...
A. Simon\textsuperscript{1,3},
D. Varga\textsuperscript{1,3},
P. Domokos\textsuperscript{1,4},
A. Vukics\textsuperscript{1$\dagger$}\orcidlink{0000-0001-8916-4033} %\textsuperscript{3$\dagger$}
%%%%%%%%%% END TODO: AUTHORS
}\end{center}
...
%{\bf 1} Affiliation1
\textbf{1} HUN-REN Wigner Research Centre for Physics, H-1525 Budapest, PO Box 49, Hungary\\\textbf{2} Center for Hybrid Quantum Networks (Hy-Q), Niels Bohr Institute, University of Copenhagen, Jagtvej 155A, Copenhagen DK-2200, Denmark\\\textbf{3} Department of Physics of Complex Systems, ELTE Eötvös Loránd University, Pázmány Péter sétány 1/A, Budapest 1117, Hungary\\\textbf{4} Department of Theoretical Physics, Institute of Physics, Budapest University of Technology and Economics, Műegyetem rkp. 3, Budapest 1111, Hungary
...
def optical_pumping(
    ...
    return tl.stack(
        tl.ramp(coil_MOTlower__A=i, coil_MOTupper__A=-i,
                duration=duration_coil_ramp, timeline=timeline),
...
\paragraph{The default} With \python{origin=None}, the reference point is the most recent \python{anchor} (\cref{sec:anchor}) if the timeline contains one, the most recent entry otherwise, and – failing both, as on an empty timeline – absolute zero, with a warning. Since every stage is recommended to end with an anchor, the practical consequence is that each \python{t} argument is read as a $\Delta t$ from the end of the preceding stage. This one default is what allows stages to be written purely in terms of their own internal logic and composed afterwards (\cref{sec:stacking}): no arithmetic inside a stage refers to anything outside it, and lengthening or inserting an earlier stage displaces everything that follows automatically, rather than invalidating a chain of hand-computed sums.
...
\end{minted}
%
This function accepts a wide range of input formats through the \python{**vtvc_dict} keyword argument, whose possibilities are laid out in \cref{tab:inputSpecs}. The arguments \python{t} and \python{context} serve as defaults for cases where time and context are not specified individually for each variable. Any other keyword is read as the name of a variable – a property put to use in \cref{sec:forwarding}.
...
\begin{table}
    ...
    \end{tabular}
    \caption{The input formats for \python{create}; see the API documentation \cite{wignertime_docs} for details.}
...
\paragraph{What a ramp refuses} A ramp must end after it begins: a zero or negative \python{duration} raises an error. A ramp whose value does not change is not an error but a hold, and occupies time like any other ramp. Since a ramp starts from the current value of its variable, ramping a variable that has not yet been set also raises an error; either set it beforehand, or state the starting point explicitly, as in the last row of \cref{tab:rampExamples}.
...
Note that \python{t} is required, and is a displacement from whatever the \python{origin} resolves to rather than an absolute time. Since the default origin is the most recent anchor where one exists, \python{t} is normally a duration measured from the end of the \emph{preceding stage} – which is precisely what allows a stage to write rows past its own closing anchor, as \python{optical_pumping} does when it reinitializes its shutters, without dragging the following stage along with them. Where the intention is instead to mark the end of everything written so far, that is a different instant and is asked for explicitly:
%
\begin{minted}[fontsize=\footnotesize, breaklines, frame=none, linenos=false]{python}
...
\end{minted}
%
The two coincide until some stage writes past its own anchor, and then they do not: in the example of \cref{sec:demonstration} they differ by about 0.1\,s from \python{optical_pumping} onwards. Which of the two is meant should therefore always be stated.
...
def MOT(duration, lower_current, upper_current, timeline=None):
    return stack(
        update(shutter_MOT=1, shutter_repump=1, timeline=timeline),
        ramp(coil_MOTlower__A=lower_current, coil_MOTupper__A=upper_current, duration=0.1),
        anchor(duration),
        context="MOT"
    )
\end{minted}
Since no timeline is supplied, this returns a deferred function that can be slotted into any later timeline. \python{MOT} also takes a \python{timeline} argument, which it passes on to its first constituent, so that it can be called on a timeline directly, just like the core functions. Such arguments are better named explicitly than collected in \python{**kwargs}: \python{update} and \python{ramp} read any keyword they do not recognize as a variable name (\cref{sec:functions}), so a misspelled argument would become a new variable without any error. The exception is the default state of the apparatus, where this is exactly what is wanted (\cref{sec:forwarding}).
...
def trigger_camera(t, exposure, context, origin=None, timeline=None) :
    return update(trigger_camera=[[t,1],[t+exposure,0]],
                  context=context, origin=origin, timeline=timeline)
\end{minted}
...
def init():
    return stack(
        create(
            ...
            coil_MOTlower__A=0, coil_MOTupper__A=0,
            t=0.0, context="initialization"
        ),
        anchor(0.0)
    ).
\end{minted}
...
timeline=stack(
    ...
    MOT(0.1,1,1),
    update(AOM_repump=0),
    trigger_camera(0.0,1e-3,"imaging")
)
\end{minted}
%
...
\end{minted}
%
This serves a similar purpose to \python{stack} at the outer layer of composition, but with the added feature that keywords are selectively forwarded to the underlying functions by name prefix.

The two take their stages differently, although their syntax does not show it. \python{stack} takes stages that have \emph{already been called} – \python{stack(timeline, MOT(duration=15))} – where every argument but the timeline has been supplied and what remains is a function of the timeline alone. \python{cascade} takes the stage functions \emph{themselves} – \python{cascade(MOT, molasses, MOT_duration=15)} – and calls them, having routed each keyword to its stage. A stage therefore reaches \python{cascade} bare and reaches \python{stack} applied, and the two are not interchangeable. What \python{cascade} returns is whatever \python{stack} makes of the first stage’s result: a timeline where that stage produces one, and otherwise a deferred function, so a \python{cascade} is itself composable. Once a good set of modular functions with default arguments is established, it is often useful to adjust specific nested parameters from a single point of contact. This is achieved by prefixing keywords with the function name, e.g.
%
\begin{minted}[frame=none]{python}
tl.cascade(
    init,
...
\end{minted}
%
Routing is strict: a keyword that matches no stage, or none of the parameters of the stage it matches, raises an error – unless that stage accepts arbitrary keywords, as \python{init} and \python{finish} do (\cref{sec:forwarding}). Where one stage name begins another, as \python{MOT} begins \python{MOT_detuned_growth} in \cref{sec:demonstration}, the longer name is tried first.
%
...
The timeline need not even be unfinished. Taking the complete \python{timeline__demo} of \cref{sec:demonstration}, with the camera trigger added to the apparatus as in \cref{sec:forwarding}, a single call triggers the camera 2\,ms after the end of the molasses stage:
\begin{minted}[frame=none]{python}
timeline = trigger_camera(2e-3, 1e-3, "imaging",
                          origin="molasses", timeline=timeline__demo)
\end{minted}
%
The exposure falls during magnetic trapping, well before the final ramps, and every row already in \python{timeline__demo} stays where it was.

This gives a practical criterion for what a stage should declare. A stage that is meant to be \emph{placed} takes \python{origin}; a stage that forms a link in a causal chain does not. The preparation sequence of \cref{sec:demonstration} is of the second kind – each of \python{MOT}, \python{molasses}, \python{optical_pumping} and \python{magnetic_trapping} acts on the physical state the previous one left behind, so there is no meaningful freedom in where it goes, and an \python{origin} argument would exist only to be given wrongly. The diagnostics of \cref{sec:forwarding} are of the first kind, and every one of them declares it. The signature therefore says which a stage is, and that is worth preserving as the operation layer grows.
 
In \cref{sec:parameter_scan}, the same mechanism places an imaging stage at a series of delays for a time-of-flight measurement.
...
In practice, few of these functions are necessary on a day-to-day basis. As demonstrated in \cref{sec:demonstration}, the main function required is \python{wignertime.adwin.core.upload}, which converts a timeline at the cycle period read from the controller itself and writes the result into the controller’s memory. It returns a record of what was written, which can be archived alongside the data.
...
loop therefore costs two comparisons per channel group, with early exit, and no
...
The arrays uploaded to the controller are prepared in the same spirit. Ramps are expanded at the cycle period of the controller and converted to DAC codes, and every row that would leave a channel at the code it already holds is then removed. What remains is one row for each change of code, at the first cycle at which that change occurs. This is the treatment of ramps suggested by Kowalski \textit{et al.} \cite{kowalski2023entangleware}, who time analog transitions at the instants when the DAC code actually changes, rather than sampling the trajectory at a fixed rate; on hardware that can act only at cycle boundaries, the two give the same result. A ramp is therefore as smooth as the hardware allows, the controller receives no more rows than are needed to describe it, and no time resolution has to be chosen.
...
A systematic review of the codebase was carried out with the assistance of Anthropic’s Claude Code, which surfaced a number of latent defects – several of them silent, in the sense that they yield a physically wrong timeline without raising an error, and had survived more than two years of daily use.
These are catalogued in the repository and are being addressed. The tool was used for review, verification and documentation; the design decisions, and the final judgement on each finding, remained with the authors.
...
\end{itemize}
Leaving \python{origin=None} does not suppress the feature, but selects the first applicable option in \python{wignertime.config.ORIGIN__DEFAULTS} – by default the most recent anchor if one exists, and the most recent entry otherwise. The options are tried in order, and should neither be available, as on an empty timeline, the new entries are placed in absolute time and a warning is emitted. The two slots are completed independently, so a partially stated origin keeps the default for the slot it leaves out – \python{origin="molasses"} passed to \python{ramp} takes its time from that context and its value from the variable itself. Consequently \python{None} in a slot means “use the default here”, and absolute placement is requested with \python{0.0}, not with \python{None}.
 
Three strings have dedicated meanings:
 
\python{"last"} – time is relative to the entry with the highest time recorded so far in the timeline.
 
\python{"variable"} – each variable is placed relative to its own most recent entry, independently of the others. This is the one reserved string admissible in either slot.
...
\python{"last"} and \python{"anchor"} are defined temporally and are admissible in the time slot only, as is a context name; the reason is given in the caption of \cref{fig:origin}. Because these three strings take precedence over any variable or context of the same name, no variable or context may be named after one of them.
...
The \python{ramp} function takes a second origin, \python{origin2}, following the same specification and governing the end point of the transition. Its default, \python{["variable", 0.0]}, refers the end point’s \emph{time} to the start point of the same variable, and its value slot to nothing at all, since the end value of a ramp is always stated. Together that is what makes \python{duration} and a target value a complete specification of a ramp. The start point is the exception: its default is \python{[["anchor", "variable"], ["last", "variable"]]}, whose value slot refers the ramp’s starting value to the variable’s own previous value. A ramp must look up where the variable currently sits, and this default is what makes ramps chain. Elsewhere no default is value-relative, and value origins are available – as in the last row of \cref{tab:rampExamples} – but are always requested explicitly.
 
\begin{figure}
  \centering
  \includegraphics[width=0.95\linewidth]{graphic/origin-resolution.pdf}
  \caption{\label{fig:origin} Resolution of an \python{origin}. The two slots admit different vocabularies because they ask different questions: the time slot asks \emph{when}, and every option shown names an instant, whereas the value slot asks \emph{how much, of what}, and only a variable names a quantity. \python{"anchor"}, \python{"last"} and a context name are therefore refused there – each would resolve to whichever variable happened to hold the row at that instant, and so would answer in the wrong physical units. Nothing is lost, since the intended reading is already expressible as a pair: the value \python{coil__A} held at the end of the molasses stage is \python{origin=["molasses", "variable"]}, the context bounding the lookup in time and the variable naming what is looked up. The two highlighted branches are the cases that occur in nearly all user code: the default, which resolves to the most recent anchor, and the resolution of a context name to that context’s anchor (\cref{sec:origin}).}
\end{figure}
...
def MOT_detuned_growth( duration=100e-3, duration_ramp=10e-3, detuning__MHz=-5, timeline=None) :
    """
...
def MOT_off(timeline=None):
    """
    respecting shutter lags
    """
    return tl.update(shutter_MOT=[-constants.lag_MOT_shutter,0], AOM_MOT=0, shutter_repump=[-constants.lag_repump_shutter,0], AOM_repump=0, timeline=timeline)
...
def pull_coils(duration, lower_current, upper_current, pt=3,
               timeline=None, t=None, context=None):
    """
...
def magnetic_trapping(
    ...
    """
    return tl.stack(
        pull_coils(duration_initial, lower_current_initial, upper_current_initial, context="magnetic_trapping", timeline=timeline),
...
machine.Start_Process(1) # assuming that the ADwin backend program is loaded on slot 1 of ADwin
\end{minted}
...
\section{Extending the default state from a downstream module}
\label{sec:forwarding}

A recurring situation in the operation layer is that a module does not describe a new experiment, but \emph{extends} an existing one with additional hardware. In our laboratory a diagnostics module adds absorption-imaging equipment – an imaging shutter, an imaging AOM and a camera trigger – to the base cold-atom experiment of \cref{sec:demonstration}. These three channels take no part in preparing the sample, but they are nonetheless part of the apparatus, and like every other channel they require a defined state at both ends of a run (\cref{sec:context}).

There are two unsatisfying ways to arrange this. The base \python{default_state} can be edited to mention the imaging hardware, which couples the base experiment to equipment it never uses and makes that one function the place where every future module must intervene. Or the diagnostics module can declare a default state of its own, which duplicates the base specification and reintroduces precisely the divergence that having a single \python{default_state} exists to prevent (\cref{sec:discussion}).

Keyword forwarding provides a third way. \python{default_state} accepts \python{**kwargs} and passes them on to its final \python{create} or \python{update} call, which reads any keyword it does not itself recognize as a variable and its value (\cref{sec:functions}). The two stages that wrap it, \python{init} and \python{finish}, forward \python{**kwargs} in the same way, so a keyword that neither consumes falls through to the default state and initializes a variable at both ends of the run. Only these three functions accept arbitrary keywords; ordinary stages name their arguments explicitly, since there a misspelled argument would otherwise become a new variable (\cref{sec:stacking}).

\begin{minted}[fontsize=\scriptsize, breaklines, breakanywhere, frame=none]{python}
# experiment.py - the base apparatus
def default_state(f=tl.create, MOT_ON=True, cavity_transmission=True, **kwargs):
    return f(
        lockbox_MOT__MHz=0.0,
        AOM_MOT=1,
        AOM_repump=1,
        shutter_MOT=int(MOT_ON),
        shutter_repump=int(MOT_ON),
        shutter_science=int(cavity_transmission),
        trigger_TC__V=0.0,
        # ... the remainder of the base state
        **kwargs,
    )

def init(MOT_ON=False, cavity_transmission=False, **kwargs):
    return default_state(
        t=-1e-6, context="ADwin_LowInit",
        MOT_ON=MOT_ON, cavity_transmission=cavity_transmission,
        **kwargs,
    )

# diagnostics.py - a module that adds absorption imaging
connections = pd.concat([
    experiment.connections,
    adcon.new(["shutter_imaging", 1, 13],
              ["AOM_imaging", 1, 5],
              ["trigger_camera", 1, 0]),
])

def init(**kwargs):
    return experiment.init(shutter_imaging=0, AOM_imaging=1, trigger_camera=0, **kwargs)

def finish(**kwargs):
    return experiment.finish(shutter_imaging=0, AOM_imaging=1, trigger_camera=0, **kwargs)
\end{minted}

The three keywords given in \python{diagnostics.init} are consumed by nothing on the way down: \python{experiment.init} passes them on, and \python{default_state} adds them to its \python{create} call. The base experiment file is untouched, the base state is stated exactly once, and \python{diagnostics.finish} does the same against \python{experiment.finish}, so that the added channels are defined at the end of a run as well as at its beginning. The connection table is extended in the same spirit – the diagnostics module declares only its own three channels and concatenates them onto the ones it inherits. This is the module referred to as \python{di} in \cref{sec:parameter_scan}.

Nothing here is a library feature. It is ordinary Python keyword forwarding, available because stages are ordinary functions rather than objects with fixed interfaces, and because \python{create}, \python{update} and \python{ramp} deliberately read any keyword they do not recognize as a variable. The cost of that openness is that a misspelled keyword reaching the final \python{create} or \python{update} call is still a legal variable name, and so creates a variable rather than raising an error. The connection table is what bounds the consequence – a variable with no matching connection cannot reach the hardware, and is removed during conversion (\cref{sec:connectionlayer}) – which is a further reason to treat that table, rather than the stage functions, as the authoritative statement of what the apparatus is.
