...
\begin{figure}
  ...
  \caption{\label{fig:origin} Resolution of an \python{origin}. The two slots admit different vocabularies. The time slot takes any of the options shown, each of which names an instant; the value slot takes a number, \python{"variable"}, or an explicit variable name. The remaining options -- \python{"anchor"}, \python{"last"} and a context name -- are refused there, because each resolves to whichever variable happens to hold the row at that instant, and so would answer in the wrong physical units. Nothing is lost, since the intended reading is already expressible as a pair: the value \python{coil__A} held at the end of the molasses stage is \python{origin=["molasses", "variable"]}, the context bounding the lookup in time and the variable naming what is looked up. The two highlighted paths are the cases that occur in nearly all user code: the default resolution to the most recent anchor, and the resolution of a context name to that context’s anchor (\cref{sec:origin}).}
\end{figure}
